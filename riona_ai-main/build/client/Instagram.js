...
const ollama_interact_1 = require("../ollama_interact"); // Ajustez le chemin si nécessaire
const agentcontroller_1 = require("./agentcontroller");
// --------------------------
// Déclaration globale des sélecteurs
// --------------------------
const postSelector = (index) => `article:nth-of-type(${index + 1})`;
...
const moreLinkSelector = 'span.x1lliihq';
// --------------------------
...
async function runInstagram(username, minPort = 8000, maxPort = 9000) {
    // Générer un
    ...
    try {
        ...
        while (true) {
            ...
            while (processedIDs.size < targetCount) {
                ...
                if (newIDs.length === 0) {
                    logger_1.default.info("No new posts, scrolling...");
                    await page.evaluate(() => window.scrollBy(0, window.innerHeight));
                    await delay(3000);
                    continue;
                }
                for (const postId of newIDs) {
                    if (processedIDs.size >= targetCount)
                        break;
                    await generateCommentForPost(page, postId, username);
                    await delay(2000); // Délai entre les deux générations
                    await generateCommentForPost(page, postId, username);
                    processedIDs.add(postId);
                    logger_1.default.info(`Processed post ${postId} as ${username} (${processedIDs.size}/${targetCount})`);
                    await delay(5000);
                }
                await page.evaluate(() => window.scrollBy(0, window.innerHeight));
                await delay(3000);
            }
            logger_1.default.info(`Completed a batch of ${targetCount} posts for ${username}. Starting a new batch...`);
        }
    }
    catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
        logger_1.default.error(`Instagram agent error: ${errorMessage}`);
    }
    finally {
        // Cleanup resources
        if (browser)
...
async function extractPostIDs(page) {
    ...
    for (let index = 0; index < posts.length; index++) {
        ...
        let postId = await page.evaluate((selector) => {
...
async function generateCommentForPost(page, postId, connectedUsername) {
    ...
    for (const sel of usernameSelectors) {
        try {
            ...
            if (usernameElement) {
                postUsername = await usernameElement.evaluate((el) => el.innerText.trim());
                if (postUsername && postUsername.trim().length > 0) {
                    logger_1.default.info(`Nom d'utilisateur du post trouvé avec le sélecteur "${sel}" : ${postUsername}`);
                    break;
                }
            }
        }
    ...
    if (!postUsername || postUsername.trim().length === 0) {
        try {
            ...
            if (postUsername && postUsername.trim().length > 0) {
                logger_1.default.info(`Nom d'utilisateur du post extrait du titre de la page : ${postUsername}`);
            }
        }
        catch (error) {
            logger_1.default.warn("Erreur lors de l'extraction du nom d'utilisateur du post via le titre:", error);
        }
    }
    const { matched, accountData } = await (0, agentcontroller_1.checkTargetUsernameMatch)(postUsername);
    if (!matched) {
        logger_1.default.info(`Post username "${postUsername}" is not in our target list. Skipping...`);
        return;
    }
    logger_1.default.info(`Post username "${postUsername}" is in our target list. Proceeding...`);
...
Original Post: "${caption}`;
    try {
        const result = await (0, ollama_interact_1.interactWithOllama)(prompt, undefined, undefined, "llama3.1:latest", false, console.log, undefined, "json", postId, caption, undefined, postUsername, // userId is undefined here
        connectedUsername // Pass username to be stored with comment
        );
        // Extraire le commentaire généré
        let extractedComment = "";
        try {
            const jsonResult = typeof result.response === "string"
